...
2. Fix every issue it reports:
   - **MISSING** — create the missing `command/<name>.md` (description copied from the skill's frontmatter, body: "Load the `<name>` skill with the skill tool, then apply its workflow to the current situation. Do not look for it inside this repo — it is an installed skill, independent of the project."), add the README.md entry, or add the `plugin.json` entry.
